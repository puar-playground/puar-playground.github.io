---
title: 🔊 FDN Reverb - Ripples of Space and Time
...
<!-- ✅ 使用统一的初始化脚本，通过 data attributes 传递配置 -->
<script 
  data-audio-a-url="https://raw.githubusercontent.com/puar-playground/FDN-Reverb/main/dry.mp3"
  data-audio-b-url="https://raw.githubusercontent.com/puar-playground/FDN-Reverb/main/wet_fdn.mp3"
  data-allow-upload="false"
...
- `x[n]`: Input signal
- 
Assuming there are 8 delay lines：
```
        +--------------------------------+
        |                                |
x[n] → +→ [delay1] → y1[n] →------------+
...
> For a fixed configuration, the FDN produces a *deterministic impulse response* (`h[n]`) — a synthetic RIR that can be used just like a recorded one.
...
🎛 What “modulation” means in a reverb context
...
**In other words**:
Modulation makes the internal delay lines breathe — introducing slight fluctuations that smear phase relationships and create a richer, more natural tail.

Which means:
The result is a diffuse, lush, chorused reverb tail — like what you hear in professional-grade reverb hardware and pedals (Lexicon, Strymon, Eventide).
...
**[FDN-Reverb – High-Performance Feedback Delay Network Reverb (PyTorch + C++)](https://github.com/puar-playground/FDN-Reverb)**  
