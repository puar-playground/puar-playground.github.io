...
def radar_plot(values, labels, instance_names, title='Nice title', color_map='default',
               fig_size=(14, 14), label_size=14, title_size=20, legend_size=14, style='seaborn-v0_8-colorblind',
               save_dir=None, labels_ticks=None):

    num_vars = len(labels)
    num_instance = len(values)
    if color_map == 'default':
        color_map = color_extend(num_instance)
    ...
    # plt.grid(color=(0.96, 0.96, 0.96), linestyle='dashed')

    for i in range(num_instance):
        v = values[i]
        v += v[:1]
        ax.plot(angles, v, color=color_map[i], linewidth=1, label=instance_names[i])
        ax.fill(angles, v, color=color_map[i], alpha=0.25)
...
if __name__ == "__main__":
    ...
    values = [[0.2 + 0.8 * random.random() for x in range(num_vars)] for i in range(n_method)]
    ...
    radar_plot(values, labels, instance_names, title=f'This is such an amazing title', color_map='default',
               fig_size=(12, 12), label_size=16, title_size=20, legend_size=16, style="seaborn-v0_8-colorblind",
               labels_ticks=labels_ticks, save_dir='test.png')
